...
void add()
{
	...
	FILE *fz;
	fz=fopen("Assignment.dat","a");	
	printf("\n");


	I:
	if((fz = fopen("Assignment.dat","r+"))!=NULL)// chechs if the file is empty
	{
	printf("\n ID         : ");
		scanf("%i",&nw.id);
...
void remove_record()
{
	...
	else
	{
		...
		else
		{
			...
			rename("Temp_file.dat","Assignment.dat");	
